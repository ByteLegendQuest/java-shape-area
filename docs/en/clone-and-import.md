...
`git clone https://github.com/ByteLegendQuest/java-shape-area`

A project directory named `java-shape-area` will be created, which can be moved wherever you like.

Open `IDEA`, `File` - `Open`, select and open the `pom.xml` in `java-shape-area` directory which is created just now.
